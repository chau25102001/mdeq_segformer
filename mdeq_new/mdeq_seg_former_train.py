...
from models.mdeq_segformer import get_seg_net
...
def main():
    ...
    final_output_dir = increment_path(config.OUTPUT_DIR, False)
    ...
    logging.basicConfig(filename='log_solver_segformer_with_pretrainsteps.txt', level=logging.INFO, format="%(message)s", filemode='w')
    ...
    # ckpt = torch.load("runs/mdeq_segformer6/checkpoint_best.pt", map_location = device)
    # print(ckpt.keys())
    # pretrain_weight = torch.load("pretrained/mit_b3.pth", map_location=device)
    ...
    #state_dict = model.module.model.fullstage.segformer.state_dict()
    #intersec = intersect_dicts(pretrain_weight, state_dict)
    #print(len(intersec.keys()))
    #model.module.model.fullstage.segformer.load_state_dict(intersec, strict=False)
