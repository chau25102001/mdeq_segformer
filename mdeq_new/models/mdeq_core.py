...
class MDEQNet(nn.Module):
    ...
    def _forward(self, x, train_step=-1, compute_jac_loss=True, spectral_radius_mode=False, writer=None, logger = None, **kwargs):
        """
        ...
        if not deq_mode:
            ...
            with torch.no_grad():
                if self.training:
                    result = self.f_solver(func, z1, threshold=f_thres, stop_mode=self.stop_mode, name="forward", writer = writer, logger = logger)
                else:
                    result = self.f_solver(func, z1, threshold = f_thres, stop_mode = self.stop_mode, name = 'forward', writer = None)                   
                z1 = result['result']
            new_z1 = z1
            ...
            if self.training:
                ...
                def backward_hook(grad):
                    ...
                    result = self.b_solver(lambda y: autograd.grad(new_z1, z1, y, retain_graph=True)[0] + grad, torch.zeros_like(grad), 
                                          threshold=b_thres, stop_mode=self.stop_mode, name="backward", writer = writer, logger = logger)
