...
from .segformer import OverlapPatchEmbed, Block
import numpy as np
...
from lib.layer_utils import list2vec, vec2list, norm_diff, conv3x3, conv5x5, flat, restore_shape
...
class MDEQModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_channels, big_kernels, dropout=0.0):
        ...
        #segformer part
        patch_embed1 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=num_channels[0], embed_dim=num_channels[1])
        patch_embed2 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=num_channels[1], embed_dim=num_channels[2])
        patch_embed3 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=num_channels[2], embed_dim=num_channels[3])
        #b0_config

        self.path_embed_list = nn.ModuleList([patch_embed1, patch_embed2, patch_embed3])
        #TODO: put this in config file later
        num_heads = [1,2,4,8]
        mlp_ratios = [4,4,4,4]
        depths = [2,2,2,2]
        sr_ratios = [8,4,2,1]
        ...
        cur = 0
        qkv_bias = True
        block1 = nn.ModuleList([Block(
            dim=num_channels[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=num_channels[1]//num_heads[1] ** -0.5,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],norm_layer=norm_layer,
            sr_ratio=sr_ratios[1])
            for i in range(depths[1])])
        norm1 = nn.LayerNorm(num_channels[1])
        cur += depths[1]

        block2 = nn.ModuleList([Block(
            dim=num_channels[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=num_channels[2]//num_heads[2] ** -0.5,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],norm_layer=norm_layer,
            sr_ratio=sr_ratios[2])
            for i in range(depths[2])])
        norm2 = nn.LayerNorm(num_channels[2])
        cur += depths[2]

        block3 = nn.ModuleList([Block(
            dim=num_channels[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias,
            qk_scale=num_channels[3] // num_heads[3] ** -0.5,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[3])
            for i in range(depths[3])])
        norm3 = nn.LayerNorm(num_channels[3])
        # cur += depths[3]
        self.block_list = nn.ModuleList([block1, block2, block3])
        self.norm_list = nn.ModuleList([norm1, norm2, norm3])
    ...
    def forward(self, x, injection, *args): #x is z and injection are lists
        """
        ...
        # Step 0: calculate attention stuff

        x_temp = [x]
        B = x.size(0)
        for i in range(1, self.num_branches):
            x_ = x_temp[-1]
            x_, H, W = self.path_embed_list[i-1](x_)
            for j, blck in enumerate(self.block_list[i-1]):
                x_ = blck(x_, H, W)
            x_ = self.norm_list[i-1](x_)
            x_ = x_.reshape(B,H,W,-1).permute(0,3,1,2).contiguous()
            x_temp.append(x_)
        # Step 1: Per-resolution residual block
        x_block = []
        for i in range(self.num_branches):
            x_block.append(self.branches[i](x_temp[i], injection[i]))
...
class MDEQNet(nn.Module):
    ...
    def _forward(self, x, train_step=-1, compute_jac_loss=True, spectral_radius_mode=False, writer=None, logger=None,
        ...
        z_list = [torch.zeros_like(elem) for elem in x_list] #the first z

        z1 = z_list[0]
        size = [z1.size(1), z1.size(2), z1.size(3)]
        z1 = flat(z_list[0])


        func = lambda z: flat(self.fullstage(restore_shape(z, size), x_list))
        ...
        y_list = self.iodrop(restore_shape(new_z1, size))
